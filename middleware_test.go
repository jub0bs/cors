			...
			cases: []ReqTestCase{
				{
					...
					category: isPreflightAndSucceeds,
					respHeaders: http.Header{
						headerACAO: {"http://localhost:9090"},
						headerACAC: {"true"},
						headerACMA: {"30"},
					...
					category: isPreflightAndFailsDuringCORSCheck,
				}, {
					...
					category: isPreflightAndFailsAfterCORSCheck,
				}, {
		...
		f := func(t *testing.T) {
			...
			for _, tc := range mwtc.cases {
				f := func(t *testing.T) {
					...
					if tc.category.isPreflight() { // preflight request
						if spy.called.Load() {
